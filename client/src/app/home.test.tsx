...
describe("Homepage / feed de posts (integracao)", () => {
  ...
  it("renderiza os posts retornados pela API", async () => {
    mockedGetPosts.mockResolvedValueOnce({
      posts: [
        { id: 1, title: "Primeiro post", body: "Corpo 1", reactions: { likes: 5, dislikes: 1 }, liked: false },
        { id: 2, title: "Segundo post", body: "Corpo 2", reactions: { likes: 8, dislikes: 3 }, liked: false },
      ],
      total: 2,
      skip: 0,
      limit: 10,
    ...
    expect(screen.getByText("👎 1 dislikes")).toBeInTheDocument();
    expect(screen.getByText("👍 8 likes")).toBeInTheDocument();
    expect(screen.getByText("👎 3 dislikes")).toBeInTheDocument();
  });

  it("usuario deslogado vê alert ao tentar curtir um post do feed", async () => {
    const alertSpy = jest.spyOn(window, "alert").mockImplementation(() => {});
    mockedGetPosts.mockResolvedValueOnce({
      posts: [{ id: 1, title: "Primeiro post", body: "Corpo 1", reactions: { likes: 5, dislikes: 1 }, liked: false }],
      total: 1,
      skip: 0,
      limit: 10,
    });
